...
(cl-defstruct (elbank-account (:constructor elbank-account-create)
			      (:copier nil))
  ...
  (iban "" :type 'string)
  (bank-name nil :type 'string)
  ;; account type from boobank:
  ;; - 1: Checking
  ;; - 2: Savings
  (type 1 :type 'number)
  (balance "" :type 'string))
...
(defun elbank-account-with-id (id)
  ...
  (seq-find (lambda (account)
	      (string= (elbank-account-id account) id))
...
(defun elbank-account-group (account)
  ...
  (cadr (split-string (elbank-account-id account) "@")))

